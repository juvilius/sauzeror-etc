...
# '''
# gaps = np.linspace(0.2, 8, 40)
# limits = np.linspace(0.2, 8, 40)
gaps = np.linspace(0.5, 1, 20)
limits = np.linspace(1.8, 3, 20)
print(len(list(Path('./table_clutter/').glob('*.44np1table'))))
sets = []
count = {'{0:.3f} {1:.3f}'.format(g,l) : 0 for g,l in itertools.product(gaps,limits)}
for p in Path('./table_clutter/').glob('*.44np1table'):
    s1 = set()
    highest = 0
    for line in open(p,'r'):
        if line[0].isdigit():
            l = line.split()
            g,l,tq,tt,length = float(l[0]),float(l[1]),float(l[2]),float(l[3]),float(l[4])
...
for pdb in random.sample(list(truth), 300):
    tr = [k for k,v in truth[pdb].items() if v != '-']
    if len(tr) > 0:
        print(qid[tr[0]], pdb)
        os.system('python 44new_params.py align /home/julius/databases/dalilite_benchmark/structure_data/scope140_pdb/{0}.ent /home/julius/databases/dalilite_benchmark/structure_data/pdb_and_scope/{1}.pdb.gz -o ./table_clutter/{0}{1}.44np1table'.format(qid[tr[0]], pdb))
