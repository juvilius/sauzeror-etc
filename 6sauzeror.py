...
parser_a.add_argument('--gap-cost', type=float, default=2.2, help='gap cost')
parser_a.add_argument('--limit', type=float, default=4.2, help='limit parameter for Smith-Waterman-algorithm')
...
class Alignment:
    ...
    def align_and_rotate(self): # get the local alignment, calculate optimal rotation matrix for structures to fit into each other
        ab = spatial.distance_matrix(self.query.er, self.target.er) # normal distribution (dmnd) or difference (dm_euclidian)
...
### PAIRWISE ALIGNMENTS
t3 = time()
if len(structures1)*len(structures2) >= 1:
    n_cores = mp.cpu_count()
    manager = mp.Manager()
    queue = manager.Queue()
