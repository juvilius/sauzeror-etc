...
class structure:
    def __init__(self, file):
        ...
        print(self.id)
        if not os.path.isfile('structures/{}.er'.format(self.id)):
    ...
    def parse_coords(self,file):
        ...
        if self.l < 7:
            ...
            with open('{}{}.er'.format('structures/', self.id), 'wt') as f:
                for atoms, er in zip(self.atoms, self.er):
                    f.write('{:05d},{},{:.3f},{:.3f},{:.3f},{:.4f}\n'.format(atoms['res_id'], atoms['res'].lower(), *atoms['coords'], er))

def write_er(s, dir='./'):
    with open('{}{}.er'.format(dir, s.id), 'wt') as f:
        for atoms, er in zip(s.atoms, s.er):
            f.write('{:05d},{},{:.3f},{:.3f},{:.3f},{:.4f}\n'.format(atoms['res_id'], atoms['res'].lower(), *atoms['coords'], er))
...
class er_structure:
    def __init__(self, file):
        ...
        self.atoms = []
        self.id = os.path.basename(file).split('.')[0]
        self.atoms, self.er = read_er(file)
        self.l = len(self.er)
        self.coordinates = np.asarray([a['coords'] for a in self.atoms])

### INITIATING STRUCTURE OBJECTS
t4 = time()
# input1 = list(Path('/home/julius/databases/dalilite_benchmark/structure_data/scope140_pdb/').glob('*.ent'))
# input2 = ['/home/julius/databases/dalilite_benchmark/structure_data/pdb_and_scope/{0}.pdb.gz'.format(p.rstrip()) for p in open('/home/julius/databases/dalilite_benchmark/pdb_and_scope.list', 'r')]
# n_cores = mp.cpu_count()
# with mp.Pool(n_cores) as pool:
#     structures1 = pool.map(structure, input2)
# # [structure(p) for p in input2]
# sys.exit()

structures1 = [er_structure(p) for p in Path('scope140-structures/').glob('*.er')]

print(time()-t4, ' s for loading all structures')
# sys.exit()
...
def sauzer(queue,q,t,gap=0.5, limit=1.4):
...
allpdb = [line.rstrip() for line in open('pdb-scope.txt', 'r')]
for chunk in chunks(allpdb, 2048):
    t3 = time()
    structures2 = [er_structure(p) for p in chunk]
    print('and another 2048 structures... took {:.2f} s this time'.format(time()-t3))
    for struc in structures1:
        t3=time()
        print(struc.id, strftime('%H%M%S'))
        output_file = open('results/'+struc.id+'-deuxieme-0.5-1.4-sauzeror.txt', 'a')

        output_file.write('\n# pre-scaled LeaderRanks, 5-14Ang, 1st principal component only\n')
        output_file.write('# chain\tl\tlali\tgaps\tscore\trmsd\tgdt_ts\tgdt_sim\ttm1\ttm2\ttm\n')
        output_file.flush()
        n_cores = mp.cpu_count()
        manager = mp.Manager()
        queue = manager.Queue()
