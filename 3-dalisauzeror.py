...
def nlocalalign(ab,gap,limit):
    ...
    for i in range(1,m+1):
        ...
        for j in range(1,n+1):
            down = f[i-1,j] - gap
            right = prev - gap
            diag = f[i-1,j-1] - ab[i-1,j-1] + limit
...
class Alignment:
    ...
    def __init__(self, query, target, gap, limit):
        self.query = query
        self.target = target
    ...
    def align_and_rotate(self): # get the local alignment, calculate optimal rotation matrix for structures to fit into each other
        ...
        self.i_list, self.j_list, self.is_gap, self.score = nlocalalign(ab,self.gap,self.limit)
...
def sauzer(queue,q,t,gap=0.8, limit=0.7, gap2=0.4, limit2=0.2):
    normi = 0
    a = Alignment(q,t,gap,limit)
    if a.tm > 0.1:
        qrotated = (q.coordinates-a.query_centroid)@a.rotation_matrix
        trotated = (t.coordinates-a.target_centroid)

        dm = spatial.distance_matrix(qrotated, trotated, p=2)
        # plt.imshow(dm); plt.colorbar();plt.title(__file__+' - 2st alignment DM - REFINEMENT'); plt.show()
        i_list, j_list, is_gap, score = nlocalalign(dm,gap2,limit2)
        ...
        if not traceback_len < 10:
            ...
            output = '{0}\t{1:04d}\t{2:04d}\t{3:03d}\t{4:.3f}\t{5:.3f}\t{6:.3f}\t{7:.3f}\t{8:.5f}\t{9:.5f}\t{10:.5f}'.format(t.id, t.l, traceback_len, nrgaps, score, rmsd_, gdt_ts, gdt_sim, tmq, tmt, tm)
        else:
            normi=1
    else:
        normi=1
...
for e in input2:
    ...
    for struc in structures1:
        ...
        output_file = open('results/'+struc.id+'-refinement-3-sauzeror-0.8-0.7-0.4-0.2.txt', 'a')
