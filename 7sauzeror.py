...
parser_a.add_argument('--gap-cost', type=float, default=2.35, help='gap cost')
parser_a.add_argument('--limit', type=float, default=6, help='limit parameter for Smith-Waterman-algorithm')
parser_a.add_argument('--gap2', type=float, default=0.1, help='gap cost')
parser_a.add_argument('--limit2', type=float, default=0.06, help='limit parameter for Smith-Waterman-algorithm')
        ...
        if not (len(i_list) < 10 or len(j_list) < 10):
            ...
            if tm > ali.tm:
                ...
                output.append(' '.join(['alignment_length:', '{:d}'.format(traceback_len),
                    ...
                    '\nTMscore (normalised by length of chain_1):', '{:.5f}'.format(tmq),
                    '\nTMscore (normalised by length of chain_2):', '{:.5f}'.format(tmt),
...
# normal
    if normi==1:
        ...
        output.append(' '.join(['alignment_length:', '{:d}'.format(ali.traceback_len),
            ...
            '\nTMscore (normalised by length of chain_1):', '{:.5f}'.format(ali.tmq),
            '\nTMscore (normalised by length of chain_2):', '{:.5f}'.format(ali.tmt),
