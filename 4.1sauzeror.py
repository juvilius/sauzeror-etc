...
class Alignment:
    ...
    def align_and_rotate(self): # get the local alignment, calculate optimal rotation matrix for structures to fit into each other
        ...
        # print(self.query.er[:10,0])
        ab = spatial.distance_matrix(scale2(self.query.er), scale2(self.target.er))
        plt.imshow(ab);plt.colorbar();plt.show()
        # print(ab[:10,:10])
...
gaps = [1]#np.linspace(0.2, 2, 19)
limits = [1]#np.linspace(0.2, 2, 19)
t3 = time()
if args.multiprocessing:
    n_cores = mp.cpu_count()
    manager = mp.Manager()
