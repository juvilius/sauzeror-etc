...
def eigenrank(atom_coordinates, cutoff=16):
    ...
    n = dm.shape[0]
    leaderranks = np.zeros((n,cutoff-4))
    eg3 = np.ones(n+1)
    eg3[n] = 0  # ground node
    for a in range(4, cutoff):
        eg1 = np.ones((n+1,n+1))
        ...
        while error > error_threshold:
            ...
            h+=1
        ground = eg2[n]/n
        leaderranks [:,a-4] = np.delete(eg2,-1)+ground
...
class structure:
    ...
    def atomium_parse(self,file):
        ...
        for chain_id, coords in self.coord_dict.items():
            self.er_dict[chain_id] = eigenrank(coords, self.cutoff)
    ...
    def parse_coords(self,file):
        ...
        self.l = self.coordinates.shape[0]
        self.er = eigenrank(self.coordinates, self.cutoff)
gcutoff = 0
for cutoff in range(6,21):
    gcutoff = cutoff
    ### OUTPUT FILE
    if args.output != False:
        output_file = open('from4{0:02d}{1}'.format(cutoff,args.output), 'w')
