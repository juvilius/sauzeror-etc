...
parser_a.add_argument('--gap-cost', type=float, default=2.4, help='gap cost')
parser_a.add_argument('--limit', type=float, default=3, help='limit parameter for Smith-Waterman-algorithm')
...
def dm_ndtr(x,y):
    ...
    if m < n:
        for i in range(m):
            result[i,:] = minkowski_distance(ndtr(x[i]),ndtr(y))
    else:
        for j in range(n):
            result[:,j] = minkowski_distance(ndtr(x),ndtr(y[j]))
    return result
def dm_scipy(x,y):
    x = np.asarray(x)
    m, k = x.shape
    ...
    if m < n:
        for i in range(m):
            result[i,:] = minkowski_distance(x[i],y,2)
    else:
        for j in range(n):
            result[:,j] = minkowski_distance(x,y[j],2)
    return result
...
def dm_euclidian(a,b):
    ...
    for i in range(m):
        # for j in range(n):
        result[i,:] = np.sqrt(np.sum((a[i,:]-b)**2))
    # else:
    #     for j in range(n):
    #         result[:,j] = np.linalg.norm(a-b[j])
    # result = spatial.distance_matrix(a,b)
...
def eigenrank(atom_coordinates, numba=1):
    ...
    def pca_correcting(leaderranks):
        ...
        i = np.argmax(lrcenter[0,:])
        print(i)
        if not np.array_equal(np.greater(lrcenter[0,i], 0), np.greater(princo[0,i], 0)):
            princo = -princo
...
def sauzer(q,t,gap=args.gap_cost,limit=args.limit):
    ...
    output.append('')
    output = '\n'.join(output)
    if (a.tm > 0.35 and args.output == False):
        print(output)
    # else:
...
def sauzer_mp(queue,q,t,gap=args.gap_cost,limit=args.limit,factor=2):
    ''' generating output for alignment '''
    a = Alignment(q,t,gap,limit,factor)
    chain_1 = ''.join([toggle_code(q.atoms[i]['res'],'3to1') if (g!=1) else '-' for i,g in zip(a.i_list,a.is_gap)])[::-1]
    chain_2 = ''.join([toggle_code(t.atoms[j]['res'],'3to1') if (g!=2) else '-' for j,g in zip(a.j_list,a.is_gap)])[::-1]
    if args.no_banner:
        output = []
...
if args.multiprocessing:
    ...
    co = itertools.product(structures1, structures2, gaps, limits)
    outputs = [sauzer(q,t,gap=g,limit=l) for q,t,g,l in co]
if args.verbose:
    t4=time()-t3
    if args.output == False:
